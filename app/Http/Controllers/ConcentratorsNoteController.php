...
class ConcentratorsNoteController extends Controller
{
    ...
    public function store(Request $request)
    {
            ...
            $payments->work_hours_output = $concentrator->work_hours;
    ...
    public function note_edit($id){
        ...
        $user = User::select('name')->where('id',$note->user_id)->first();

        return view('concentrator.note.edit.index', compact('note', 'concentrator', 'payments','user'));
    }

    public function return_concentrator(Request $request, ConcentratorNote $note_id){
        $currentDate = date('Y-m-d H:i:s');
        $concentrator =  Concentrator::find($note_id->concentrator_id);
        $concentrator->work_hours = $concentrator->work_hours + $request->hours_works_consumed ;
        $concentrator->status = 'ALMACEN';
        $concentrator->save();

        $note_id->hours_works_consumed = $request->hours_works_consumed;
        $note_id->return_concentrator = $currentDate;
        $note_id->save();

        return response()->json(['note_id' => $note_id, 'concentrator' =>$concentrator->serial_number]);
    }

    public function close(ConcentratorNote $note_id){
        $note_id->status='CERRADA';
        $note_id->save();
    }
